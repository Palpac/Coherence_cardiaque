...
public class MainActivity extends AppCompatActivity {
    ...
    Toolbar toolbar;
    Menu menu;
    AudioManager audioManager; // For volume control
    Boolean autostart, save_volume, timer_started = false, saved_playing_state, dark_theme;
    int INTENT_RETURN_ID; // Used by intent to start Settings activity
    int save_volume_value, timer_time;
    ...
    private BroadcastReceiver mNoisyReceiver = new BroadcastReceiver() { // Init headphones unpglugged receiver
        ...
        public void onReceive(Context context, Intent intent) {
            mService.PausePlayer(); // Pause player if headphones unplugged
        }
    };
    ...
    protected void onResume() {
        ...
        localBroadcastManager.registerReceiver(broadcastReceiver, broadcast_intentfilter); // To receive playback changes again
    }
    @Override
    protected void onPause() {
        //Toast.makeText(getApplicationContext(), "onPause MainActivity", Toast.LENGTH_SHORT).show();
        localBroadcastManager.unregisterReceiver(broadcastReceiver); // No need to receive playback change (button invisible)
        super.onPause();
    }
    @Override
    protected void onDestroy() { // when user end app or system destroy for memory use.
        ...
        unregisterReceiver(mNoisyReceiver); // Unregister headphones unplugged RECEIVER
    ...
    private ServiceConnection mConnection = new ServiceConnection() { // Needed Callbacks for service binding, passed to bindService()
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            mBound = true;
            boolean Player_isPlaying_State = mService.getPlayer_isPlaying_State(); // Call playing state
            if(Player_isPlaying_State) {
                if (!timer_started) {           // If button does not indicate time
                    buttonplay.setText("STOP"); // Update text button
                }
            }
            else {
                if (!timer_started) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        audioManager = (AudioManager)getSystemService(Context.AUDIO_SERVICE); // For volume control

        //////////////////////////////////////////////////////////////////////////////////////////// Headphone unplugged RECEIVER
        IntentFilter noisy_filter = new IntentFilter(AudioManager.ACTION_AUDIO_BECOMING_NOISY);
        registerReceiver(mNoisyReceiver, noisy_filter);
        ...
        broadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive( Context context, Intent intent ) {
                if (intent != null){
                    ...
                    if (intent.getStringExtra("playbackState").equals("True")) {
                        if (!timer_started) {           // If button does not indicate time
                            update_volume();
                            buttonplay.setText("STOP"); // Update button text
                        }
                    }
                    if (intent.getStringExtra("playbackState").equals("False")) {
                    ...
                    if (intent.getStringExtra("playbackState").equals("Cancel")) {
                        finish(); // Quit app
                    }
                }
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {                 // Return from Settings
        ...
        saved_sound = preferences.getString("Save_current_sound", "Original"); // Read previous sound
        sound = preferences.getString("Sound", "Original"); // Read new sound
    ...
    private void maj_preferences() { // Read user saved values
        ...
        save_volume_value = preferences.getInt("Save_volume_value", 50);
    ...
    private void update_volume() {                                                                  // Update
        if (save_volume){
            audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, save_volume_value, 0);
            //Toast.makeText(getApplicationContext(), "Volume updated : " + save_volume_value, Toast.LENGTH_SHORT).show();
        }
    }
    private void save_volume() {                                                                    //save
        save_volume_value = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC); // Get music volume value
        //Toast.makeText(getApplicationContext(), "Volume read : " + save_volume_value, Toast.LENGTH_SHORT).show();
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(this); //  Dans preferences
        SharedPreferences.Editor editor = preferences.edit(); // Mode edition
        editor.putInt("Save_volume_value", save_volume_value); // Save value
        editor.apply(); // Save
    }
